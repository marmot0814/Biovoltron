 ...
 *
 * - class R "record": must be FastaRecord or FastqRecord (inherit from
 * FastaRecord)
 * - every single genome(reads) will record one FastaRecord(FastqRecord)
 * - All sequences will record into record.seq
 * - if FastqRecord type, program will check record.DELIM and record quality of
 * reads into record.qual
 */
template<class R>
 ...
 * - if FastqRecord type, program will check record.DELIM and output quality of
 * reads in record.qual.
 */
template<class R>
  requires std::derived_from<R, FastaRecord<R::encoded>>
inline auto&
